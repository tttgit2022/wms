...
function Dynamsoft_OnReady() {
    ...
    if (DWObject) {
        if (DWObject.ErrorCode == 0) {
            ...
            var twainsource = document.getElementById("webcamsource");
...
function InitWebcam() {
    ...
    if (selWebcamSource) {
        for (var i = 0; i < arySource.length; i++)
            selWebcamSource.options.add(new Option(arySource[i], arySource[i]), i + DWTSourceCount); // Get Webcam Source names and put them in a drop-down box
    ...
    webcam_source_onchange(arySource.length);
